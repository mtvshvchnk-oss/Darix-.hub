...
-- ПОИСК ВСЕХ ВОЗМОЖНЫХ ИВЕНТОВ СТРЕЛЬБЫ В ИГРЕ
local gunEvents = {}
pcall(function()
    for _, o in ipairs(Rep:GetDescendants()) do 
        if o:IsA("RemoteEvent") then
            local n = o.Name:lower()
            if n:find("gun") or n:find("shoot") or n:find("fire") or n:find("revolver") or n:find("bullet") or n:find("kill") then 
                table.insert(gunEvents, o)
            end
        end 
    end
end)
                ...
                if hrp and hum and hum.Health > 0 then
                    ...
                    if targetCoin then
                        ...
                        while farm and targetCoin and targetCoin.Parent and hrp and hum.Health > 0 do
                            ...
                            if alpha >= 1 or (hrp.Position - endPos).Magnitude < 2 then
                                break
                            end
                        end
                        task.wait(0.05)
...
-- ПОИСК УБИЙЦЫ (Максимально жесткий сканер)
...
-- УЛЬТИМАТИВНЫЙ ТАЧ/КЛИК АИМ (Бьет сразу всеми способами)
UIS.InputBegan:Connect(function(input, gp)
        ...
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            pcall(function()
                ...
                if cChar and currentMurderer:FindFirstChild("Head") then
                    ...
                    -- Метод 1: Запуск во все найденные RemoteEvent (один из них точно сработает)
                    for _, ev in ipairs(gunEvents) do
                        pcall(function()
                            ev:FireServer(targetHead.Position, targetHead)
                            ev:FireServer(targetHead.Position)
                            ev:FireServer(targetHead)
                        end)
                    end
                    
                    -- Метод 2: Прожатие инструмента в руках (эмуляция реального клика пушки)
                    local equippedTool = nil
                    for _, tool in ipairs(cChar:GetChildren()) do
                        if tool:IsA("Tool") and (tool.Name:lower():find("gun") or tool.Name:lower():find("revolver") or tool.Name:lower():find("sheriff") or tool.Name:lower():find("ган")) then
                            equippedTool = tool
                            break
                        end
                    end
...
RS.RenderStepped:Connect(function(dt)
    ...
    pcall(function()
        if screenStretch then
            ...
            if Camera.FieldOfView == 110 then
                Camera.FieldOfView = 70
            end
        end
    end)

    if cChar and cChar:FindFirstChildOfClass("Humanoid") and cChar:FindFirstChild("HumanoidRootPart") then
        if speedHack and not noclip then
            local hum = cChar.Humanoid
            if hum.MoveDirection.Magnitude > 0 then
                local hrp = cChar.HumanoidRootPart
                local targetPos = hrp.Position + (hum.MoveDirection * (speedVal / 55))
                hrp.CFrame = CFrame.new(targetPos, targetPos + hrp.CFrame.LookVector)
            end
        elseif speedHack and noclip then
            local hum = cChar.Humanoid
            if hum.MoveDirection.Magnitude > 0 then
                cChar.HumanoidRootPart.CFrame = cChar.HumanoidRootPart.CFrame + (hum.MoveDirection * (speedVal / 55))
            end
        end
    end

    if noclip and cChar then
        for _, part in ipairs(cChar:GetDescendants()) do
    ...
    if cChar then 
        for _, i in ipairs(cChar:GetChildren()) do 
            if i:IsA("Tool") and (i.Name:lower():find("gun") or i.Name:lower():find("revolver") or i.Name:lower():find("sheriff")) then 
                has = true 
            end 
        end 
    end
